...
async def on_message(message):
    ...
    try:
        ...
        if desc.startswith('Guess the pokémon'):
            ...
            identity = text[int(index)+40:int(index)+150000]
            ...
            while True:
                ...
                new_name = identity[:identity.index('"')]
                try:
                    new_name = new_name[:new_name.index('_(')]
                except ValueError:
                    print(new_name)
                
                if new_name != name and new_name.isalpha():
                    break
            if not new_name.isalpha() or new_name==name:
                return
                    
            msg = ('If not, try: '+new_name).format(message)
            await client.send_message(message.channel, msg)
            if catch:
                poke_catch(new_name)
